...
_PAGE_HTML = (
    ...
    "  if(!confirm('将清除 HA 中两个累计 sensor 的现有 statistics，并从 fork 数据按月重建到上个月底。继续？'))return;"
    ...
    "    const lines=[];"
    "    if(data.success){"
    "      if(data.cleared_first)lines.push('（已清除现有 statistics 并重建）');"
    "      lines.push('用电量：候选 '+u.candidates+' 已有 '+u.existing+' 新增 '+u.imported);"
    "      lines.push('电费：候选 '+c.candidates+' 已有 '+c.existing+' 新增 '+c.imported);"
    "      const dim=data.current_month_daily_imported||0;"
    "      const drow=data.daily_rows||0;"
    "      if(dim>0){lines.push('本月按天补全：'+dim+' 天 (daily 表共 '+drow+' 行)');}"
    "      else if(data.current_month_daily_skipped_reason){"
    "        lines.push('本月按天未补全：'+data.current_month_daily_skipped_reason);"
    "      }"
